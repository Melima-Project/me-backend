...
module.exports = function(Order) {
    ...
    Order.validatesLengthOf('Name', {min: 3, max: 64, message: {min: 'Name is too short', max: 'Name is too long'}});
    Order.validateAsync('Price', validatePrice, {message: 'Price should be > 1'});
    ...
    Order.validatesLengthOf('SerialNumber', {min: 0, max: 20, message: {min: 'SerialNumber is too short', max: 'SerialNumber is too long'}});
    ...
    Order.validatesLengthOf('ClientLastName', {min: 3, max: 64, message: {min: 'Name is too short', max: 'Name is too long'}});
...
function validateCountAvailable(err, done) {
    ...
    process.nextTick(function () {
        if (CountAvailable >= 1 && CountAvailable <= 10000) {
        done();
        } else {
